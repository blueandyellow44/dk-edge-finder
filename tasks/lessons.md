...
## 2026-04-10: Stop trial-and-error git commands — read the error and fix the ROOT
**Mistake:** Ran `git pull --rebase && git add ... && git commit && git push` as one chain without first checking `git status`. Hit "cannot pull with rebase: You have unstaged changes." Reflexively followed with `git stash -u && git pull --rebase && git stash pop` — which worked, but only because stash/pop happened to be safe here. This was two commands where one would have sufficed. Trial-and-error git is reckless: stash can silently drop hunks on conflict, rebase can rewrite history, and chaining destructive ops masks what went wrong.
**Rule:** Before running any multi-step git chain, think through the preconditions. If I'm about to pull/rebase, I must know whether the working tree is clean. The correct flow when I have uncommitted changes I want to commit is: `git add <files> && git commit -m "..." && git pull --rebase && git push` — commit FIRST, then rebase on top. Stash is for changes I don't want in this commit, not as a workaround for ordering mistakes.
**Broader rule:** When a command fails, read the error and fix the root cause. Don't fire a second command hoping it works around the first failure. Every "sloppy error by trial and error" compounds — each retry either masks a real problem or risks silent data loss.

## 2026-04-10: Two sources of truth for bets caused silent Activity view drops
**Mistake:** Site Activity view reads `data.json.bets`. Manual override entries (and anything entered during recovery sessions) were written to `bankroll.json.resolved_bets` but never mirrored into `data.json.bets`. Result: 9 bets from 4/7-4/8 were counted in lifetime totals and bankroll math, but completely invisible in the Activity UI. User noticed: "activity doesn't show any wins/losses after 4/6."
**Rule:** `bankroll.json.resolved_bets` is canonical. `data.json.bets` is a view for the site. Whenever either `scan_edges.py` or `resolve_bets.py` runs, they must sync missing `bankroll.resolved_bets` entries into `data.json.bets` (idempotent, keyed on `(date, pick)`). This sync is now in both scripts. `resolve_bets.py` has an early return when nothing is resolved — the sync write path must also fire on that early-return branch, otherwise synced-only runs silently lose the update.
**Broader rule:** When there are two stores that must stay consistent, EVERY write path to either must reconcile the other, AND every code path that exits early must still flush reconciliation. An early-return that skips the write is the same bug as not syncing at all.
