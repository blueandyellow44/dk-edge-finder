...
## What you are inheriting (2026-05-07 AM, Session 12 close — auth-gate fix uncommitted, cron broken, slaughter was variance)

Full per-session detail: [`.claude/handoffs/2026-05-07-070532-session-12-auth-gate-fix-and-slaughter-debug.md`](.claude/handoffs/2026-05-07-070532-session-12-auth-gate-fix-and-slaughter-debug.md). This summary is the TL;DR.

**Three things to know before doing anything else:**

1. **The cron is fine. The local clone is 11 commits behind `origin/main`.** Earlier in this session I asserted "the cron has not run since 2026-05-04 16:30 UTC" based on local-file mtimes; that was wrong. `gh run list` confirms `Resolve Bets` ran successfully on 5/5 07:19, 5/6 07:35, 5/7 07:43; game scans ran multiple times each day; commit `d56281b` "Auto-resolve: 2026-05-05 bet results" already updated bankroll/data/daily_summaries/pick_history with the 5/4 MLB outcomes. Local files look frozen because the local repo hasn't been pulled. **First action next session: `git pull` from `~/Betting Skill/` (the only WIP is the uncommitted `index.html` auth-gate fix, which the pull won't touch). The one real failure was a transient GitHub-side runner availability issue on the 2026-05-05 15:05 Full Scan (annotation: "The job was not acquired by Runner of type hosted"), self-recovered on the next schedule. Net effect of that single failure: 5/5 morning's prop refresh is missing, games scans were unaffected.

2. **Uncommitted auth-gate fix sits in `index.html` working tree (+115/-23).** Patches the silent-fail mode where the Place button POSTs to a Cloudflare-Access-gated `/api/place-bets`, the cross-origin redirect fails CORS, the fetch rejects with a generic "network" toast, and `queueFailedSync()` quietly stashes the bet in localStorage with a misleading "retry from Settings" message — which is why Max's 5/5 bets did not log. The fix uses `redirect: 'manual'` to detect opaqueredirect responses, surfaces a sticky red banner and a "Sign in required, bet was NOT placed" toast, and stops `getFailedSyncQueue` from filtering out cross-day entries so yesterday's queue stays visible. NOT verified with `wrangler dev`. NOT deployed. Brace balance even (209/209), `node --check` clean. Verify with `npx wrangler dev` from `~/Betting Skill/`, walk the OTP flow end-to-end in incognito, then `npx wrangler deploy`.

3. **The "slaughter" Max raised was mostly variance plus visibility loss, not model failure.** Lifetime is +$102.68 / +10.18% ROI on 68 settled bets; paper P/L on 904 picks is +$657. May 3 was a real left-tail day (model 9W-12L = 43% on 21 picks, Max placed 6 and lost $39.58 on heavy-juice MLB +1.5 favorites and a Bundesliga over). May 4 looks like nothing because the resolver hasn't graded those bets. May 5 looks like nothing because of the auth-gate bug we just patched. Three lifetime calibration issues separate from the slump and worth addressing later: NBA is 46.8% on 205 picks (-$240), the 5-8pp edge bucket is 53.1% on 324 picks (-$225), and HIGH-confidence tier (53.9%) is inverted vs MEDIUM (59.8%, +$686).

**Discovered, contradicts an earlier claim in this HANDOFF:** Cloudflare Access now covers `/`, not just `/api/*`. `curl -sI https://dk-edge-finder.max-sheahan.workers.dev/` returns 302 with `redirect_url=%2F` and `www-authenticate: ... resource_metadata=".../.well-known/cloudflare-access-protected-resource/"` (no `/api/` segment). The Step 8 section below still says `path 'api/*'`. Either the policy was tightened post-May 1 or it was always broader than documented. Net: the legacy `index.html` only loads when you are already authed, so the fix's preflight check is mostly insurance against mid-session cookie expiry — but the Place-fetch opaqueredirect detection is still load-bearing.

**Side work this session, unrelated to the main thread:** The standalone `dk-edge-finder` domain skill at `~/.claude/skills/dk-edge-finder/scripts/` got refactored. Two duplicate edge engines were merged into `edge_calculator.py` (added `edge`, `edge_pp`, `ev_per_dollar`, `is_strong_edge` fields; fixed a silent alt-line key collision by adding `point` to the model index key; added prefix-aware tier classification). New scripts: `settle_picks.py` (settles h2h/spreads/totals from The Odds API scores endpoint and emits a calibration report by tier/edge bucket/sport with CLV when closing odds are present) and `capture_closing_lines.py` (snapshots DK odds within ±N min of `commence_time` for cron-driven CLV capture). SKILL.md updated to merge Steps 7 + 8. All smoke tests passed. **This is the example/teaching skill, not the production scanner; production stays at `~/Betting Skill/scripts/`.**

**Immediate next-step priority order:** (1) diagnose why the cron stopped on 5/4 — start with `gh run list`, then check workflow file and secrets; (2) once cron is back up, manually re-trigger `resolve_bets.py` for 5/4 + 5/5; (3) verify the `index.html` fix with `wrangler dev` in incognito; (4) `wrangler deploy`; (5) walk Max through `JSON.parse(localStorage.getItem('dk-edge-sync-queue'))` in DevTools to recover any 5/5 bets still in browser storage.
...
**Override dropped + Available/Profit/Chart reconciled (2026-05-04 session 14 close):** Six commit, post-HANDOFF: Max flagged that Available ($588.68 override), Profit (+$147.11), and Chart endpoint ($647.11) showed three different stories. Math: $500 + $147 = $647, not $588 - the $58 gap was the override. Per Max's call, dropped the KV `balance_override:max.sheahan@gmail.com` record (via `wrangler kv key delete --namespace-id ...`), cleared the stale `bankroll.json.balance_override` field (was $679.34 from the 4/10 manual edit), and refactored `worker/lib/bankroll.ts` `sumActiveStakes` to read pending bets from `data.json.bets[]` instead of KV state placements - single source of truth for active stakes, and the going-forward sync_kv_placements.py cron always writes wager into the new bets[] entry so legacy pre-session-13 records (which lack wager) still subtract correctly. Reconciled state: Available $549.11 (= $647.11 file current - $98 in 7 pending 5/4 bets); Profit +$147.11; Chart endpoint $647.11; Available + Pending = Chart endpoint = Starting + Profit. All three derive from one source. Tests rewritten: dropped the legacy "no wager doesn't subtract" test (no longer relevant since we don't read wager from KV placements), updated the override + active-stakes tests to use data.json bets entries. 83/83 still passing. Trade-off Max accepted: SPA's Available will now be lower than his actual DK balance because of ~$58 of untracked DK activity (parlays, props, in-game bets, deposits/withdrawals) that the model never sees. Resolution path discussed - email parsing of DK confirmation/settlement emails via Gmail API is the next-session lead. Override capability stays available (Account tab can set/clear); only the current value is removed. Commit: `c020ba7`. Cloudflare auto-deploys.

**Bankroll staleness fixed + Activity redesign + AI commentary + going-forward KV bridge (2026-05-04 session 14):** Closed the lead item from session 13's handoff and three new asks Max layered on. (1) **Bankroll/chart staleness root-caused and fixed.** The structural gap: SPA writes placements to Cloudflare KV; `resolve_bets.py` only reads `data.json.bets[]` / `bankroll.json.pending_bets[]`; KV placements never flowed back. Cron had been running cleanly every day (15 successful runs in the prior 15 days, all under 20s, zero diffs because the script had nothing pending to grade). One-shot script `scripts/backfill_kv_placements.py` pulled 13 placements from production KV (`state:max.sheahan@gmail.com:2026-05-03` and `2026-05-04`), joined to `pick_history.json` for outcomes, used Max's stated $14 flat stake, and appended to `data.json.bets[]`. Recomputed lifetime stats consistently from `data.json.bets[]` (single source of truth): pre-fix `data.json.bets[]` summed to $186.69 / 40W-21L-1P / 62 bets but `bankroll.json` declared $179.34 / 37W-21L-1P / 43 lifetime_bets - 16-bet drift baked in since the April 10 manual edit. Post-fix: 42W-25L-1P resolved across 67 bets, lifetime_profit $147.11, current_bankroll $647.11, 7 pending. Override stays at $588.68 per Max's call (~$58 file-vs-override gap is unexplained; possible 4/10 overstatement or untracked DK activity). BalanceChart now extends MAR 15 to MAY 3. (2) **Soccer STATUS_FULL_TIME resolver bug fixed.** `resolve_bets.py:53` only treated ESPN's `STATUS_FINAL` as final, but ESPN tags soccer with `STATUS_FULL_TIME`. So no soccer game has graded through the cron since launch - 97 stalled soccer entries pending in `pick_history.json`. One-line fix; next cron tick will resolve them all. Patched the Bundesliga 5/3 OVER 2.5 (Wolfsburg 1, Freiburg 1, total 2 -> LOSS) manually so `pick_history.json` agrees with `data.json.bets[]`. (3) **Activity tab redesign.** Rows are now click-to-expand buttons. Bets grouped by date with a per-day summary header showing W/L count and net P/L color-coded. Expanded view is Positions-style: Market / Model% / Implied% / Edge / Tier / Confidence / Wager / Win amount / Final score / full model commentary in a tier-colored card (green/gold/grey left border). Pre-pick_history bets fall back gracefully to "No model details on file" empty state. Worker enrichment: `pick_history.json` symlinked into `public/`, allow-listed in `.assetsignore` (Cloudflare Access scope is `/*` so still auth-gated). New `loadPickHistory(env)` in `worker/lib/picks.ts`; `worker/lib/activity.ts` builds a `${date}|${pick}|${event}` index and joins each normalized bet, attaching only fields actually present and parseable. `coercePercent` handles `pick_history.json`'s mix of percent-strings ("64.5%") and bare numbers (5.4). `ResolvedBetSchema` gains optional metadata fields. (4) **AI daily summary scaffolding.** `scripts/generate_daily_summary.py` runs after `resolve_bets.py` in the cron; for each date with resolved bets that doesn't already have a summary in `daily_summaries.json`, sends the day's bet shape (sport, pick, odds, wager, outcome, pnl, edge, tier, confidence, model%) to claude-haiku-4-5 via Anthropic API and stashes a 2-3 sentence commentary. Idempotent - skipped dates cost nothing. Defensive - missing `ANTHROPIC_API_KEY` logs and skips so cron keeps running. Worker `getActivityResponse` loads + normalizes summaries in parallel with data.json and pick_history; `ActivityResponseSchema` gains a `summaries` record. Frontend renders an "AI" badge + prose block under each day-group header when a summary exists. **To activate Max needs to add `ANTHROPIC_API_KEY` as a GH Actions repo secret.** (5) **Going-forward KV bridge.** `scripts/sync_kv_placements.py` runs at the top of the resolve-bets cron via Cloudflare API, lists state records for `max.sheahan@gmail.com`, appends new placements to `data.json.bets[]` as pending. `resolve_bets.py` then grades them. Idempotent on `(date, pick)`. Wager comes from `placement.wager` when present (post-session-13 records); $14 fallback for legacy. Defensive - missing CF creds, HTTP errors, missing pick_history matches all log + skip. **To activate Max needs to add `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` as GH Actions repo secrets.** Replaces the manual one-shot backfill path. Five commits on `main` this session: `609eb0e` (bankroll backfill + soccer fix), `215e5f9` (Activity tab redesign + worker enrichment), `2d2c6ed` (AI summary scaffolding), `bb0fb3f` (KV sync going forward), and the HANDOFF entry. Bundle: `index-D059TUWq.js` 251.48 kB raw / 76.50 kB gzip; CSS `index-2opY7Xk6.css` 18.76 kB raw / 3.73 kB gzip. `npx tsc -b` clean, `npm test` 83/83. Cloudflare auto-deploys from main directly.
...
## 2026-05-04 session 14 (bankroll staleness fix + Activity redesign + AI commentary + KV bridge)

### Goal
Lead item from session 13's handoff: production `bankroll.json` and the BalanceChart were frozen at April 8 even though Max kept clicking "Mark as placed" through 5/3 and 5/4. Investigate `scripts/resolve_bets.py` end-to-end, find the gap, fix it. Mid-session Max layered three asks: Activity tab rows weren't clickable; he wanted Positions-style detail (deleted in session 13) integrated into Activity; daily W/L recap with AI summary and analysis. All five things landed.

### Pre-flight
- `git stash list`: empty. `git status --short`: 3 expected carry-over untracked.
- Branch `main`. `git log --oneline -5`: top is `17db4b9 chore: HANDOFF session 13` then `9310fcd feat: bug 2 + ...` (session 13 close).
- `npx tsc -b`: clean. `npm test`: 83/83. `curl -sI .../api/me`: 302 to Access.
- `grep -c "americanWinAmount" public/assets/*.js` returned 0, not the 1 expected by session 13's handoff. Investigated: Vite mangles export names during minification, so the literal string check is a bad sentinel. Real check (rebuild + diff) confirmed bundle is in sync. Updated session 14's "if you have one minute" line to use a non-minified-affected check.
- Live deploy `1105f2b6` at 2026-05-04T16:25:16Z; CI bundle guard passed.

### Diagnosis: bankroll/chart staleness root cause

`data.json.bets[]` had 62 entries dating 3/15 to 4/8 (40W-21L-1P, sum pnl $186.69). `bankroll.json.resolved_bets[]` had 43 entries declaring $179.34 lifetime profit and 37W-21L-1P stats. Both files last touched April 10 via a manual `21feca9 [fix] Record 4/7-4/8 manual bets + fix resolve_bets pick_history skip` commit; the `lifetime_bets: 43` vs `wins+losses+pushes = 59` mismatch was the manual edit's fingerprint.

Cron history: `gh run list --workflow=resolve-bets.yml --limit 15` showed 15 successful runs since 4/20, every one completing in 9-19s with zero commits (gated by `git diff --quiet`). Script was running cleanly and producing nothing.

Read `scripts/resolve_bets.py` end to end. Main loop iterates `data.json.bets[]` looking for `outcome == "pending"`. None of the post-4/10 bets ever entered `data.json.bets[]` because: SPA's "Mark as placed" button writes placements to Cloudflare KV (`state:max.sheahan@gmail.com:<scan_date>`); resolver only reads filesystem files. KV placements were invisible to the cron.

Confirmed by inspecting prod KV directly: `npx wrangler kv key list --binding EDGE_STATE --remote` returned 3 keys total. `state:max.sheahan@gmail.com:2026-05-03` (6 placed + 1 skipped), `state:max.sheahan@gmail.com:2026-05-04` (7 placed), `balance_override:max.sheahan@gmail.com` ($588.68). Max confirmed via `AskUserQuestion` that he'd been clicking Mark as placed in the SPA, and that there was a gap between 4/8 and 5/3 where he placed nothing - the chart's frozen-on-April-8 state was actually correct for "last day of betting" but never extended forward when activity resumed.

Side finding: `pick_history.json` had 916 entries through today (5/4) and was being resolved fine on every cron tick. The 4/10 commit's "fix resolve_bets pick_history skip" had decoupled pick_history resolution from bet resolution, so paper picks kept grading even when no real bets needed grading.

Second side finding while cross-referencing: 97 pending soccer entries in `pick_history.json` with `final_score` populated by the model's morning scan but `outcome` still "pending". Looked at `resolve_bets.py:53`: `is_final = status == "STATUS_FINAL"`. Pulled the Wolfsburg/Freiburg game from ESPN; status was `STATUS_FULL_TIME`, not `STATUS_FINAL`. Soccer scores never graded. One-line fix.

### Decisions made

- **Backfill from KV + pick_history join, $14 flat stake, single-source-of-truth recompute.** Asked Max via `AskUserQuestion` what he staked on 5/3; he said $14 flat. One-shot `scripts/backfill_kv_placements.py` joins each placement to `pick_history.json` for outcome + odds, computes pnl at $14 stake, appends to `data.json.bets[]`, then recomputes lifetime stats from the resolved set so `data.json.bets[]` is the only source. bankroll.json's lifetime fields derive from that. Idempotent on `(date, pick)`.
- **Override stays at $588.68 per Max.** File-derived current is $647.11 after backfill; Max's manual override is $588.68. ~$58 unexplained drift. Asked Max: keep override at $588.68 because DK reality is the truth and the data may have overstated profit somewhere along the way. He'll readjust if it gets in the way.
- **Bundesliga 5/3 manually patched in pick_history.json.** Final score Wolfsburg 1, Freiburg 1, total = 2 -> OVER 2.5 = LOSS. Patched directly because the cron resolver missed it (soccer status bug). Tomorrow's cron tick won't double-resolve since the entry is now `outcome: loss`.
- **Activity tab join is server-side, via pick_history symlinked into public/.** Considered (a) embed pick_history fields into data.json.bets[] at scan/resolve time (denormalize) versus (b) symlink + .assetsignore + worker join. Picked (b): single source of truth, one-place enrichment, graceful fallback when no match. 535 KB load on every /api/activity call is a small latency hit for cleaner data flow.
- **AI summaries: cron-precomputed via Anthropic API, stashed in daily_summaries.json sidecar.** Asked Max which generation strategy he wanted; he picked the cron path. claude-haiku-4-5 for cheap fast 2-3 sentence commentary. Idempotent: dates already summarized are skipped on the next cron run, so re-running costs nothing. Graceful skip when ANTHROPIC_API_KEY is missing - cron stays green while Max wires the secret.
- **Going-forward KV bridge in cron, separate Python script.** `sync_kv_placements.py` lives next to resolve_bets.py and runs first in the cron sequence. Cloudflare API token + Account ID via GH secrets. Skipped vs integrated-into-resolve_bets to keep concerns separable; if KV API errors, only this step fails, resolve still runs.

### Files modified

| File | Changes | Rationale |
|------|---------|-----------|
| `scripts/backfill_kv_placements.py` | NEW (170 lines) | One-shot bridge from KV state records to data.json.bets[] |
| `scripts/sync_kv_placements.py` | NEW (180 lines) | Going-forward equivalent that runs in cron via Cloudflare API |
| `scripts/generate_daily_summary.py` | NEW (170 lines) | AI commentary generator |
| `scripts/resolve_bets.py` | +1 / -1 | Soccer STATUS_FULL_TIME bug fix |
| `data.json` | +13 bets entries + recomputed bankroll fields | Backfill |
| `bankroll.json` | Lifetime fields recomputed consistently | Backfill |
| `pick_history.json` | Bundesliga 5/3 patched to loss | Resolver-missed manual fix |
| `daily_summaries.json` | NEW (empty `{}`) | Sidecar populated by cron when API key set |
| `public/pick_history.json` | NEW symlink -> ../pick_history.json | Worker reads via env.ASSETS |
| `public/daily_summaries.json` | NEW symlink -> ../daily_summaries.json | Same |
| `.assetsignore` | +2 allow-list entries | pick_history.json + daily_summaries.json |
| `.github/workflows/resolve-bets.yml` | +2 steps (sync + summary) | Cron pipeline updates |
| `shared/schemas.ts` | +7 optional fields on ResolvedBetSchema; new DailySummarySchema; ActivityResponse gains summaries record | Schema for enrichment |
| `worker/lib/picks.ts` | +2 helpers (loadPickHistory, loadDailySummaries) | Reused by activity worker |
| `worker/lib/activity.ts` | +60 lines (buildPickHistoryIndex, normalizeSummaries, enrichment join) | Worker enrichment |
| `frontend/src/lib/format.ts` | +formatDayHeader | "2026-05-03" -> "Sun, May 3" |
| `frontend/src/tabs/ActivityTab.tsx` | Full rewrite (group by date, click-to-expand, day summary, AI block) | Layout redesign |
| `frontend/src/styles.css` | +180 lines for new Activity layout, dropped old grid table | New CSS for grouped/expandable rows |

### Verification

- `npx tsc -b`: clean.
- `npm test`: 83/83 across 3 files (existing tests unmodified; new loadPickHistory + loadDailySummaries mock-out gracefully via the existing 404 fallback in activity.test.ts's makeEnv, so summaries land as `{}` and bets land without metadata - schema still parses).
- Backfill verification: ran `python3 scripts/backfill_kv_placements.py`. 13 bets added (5/3 = 2W-4L counting Bundesliga manual loss, 5/4 = 7 pending). Lifetime: 42W-25L-1P, profit $147.11, current $647.11.
- Browser preview verification (vite-dev port 5173 + wrangler-dev port 8787): chart extends MAR 15 to MAY 3 with delta +$147.11 over 13 days. Click on Rockies row in Activity tab expands to: Market Spread / Model 61.3% / Implied 52.8% / Edge 8.4% / Tier High / Confidence LOW / Wager $14 / Win amount $12.50 / Final ATL 11, COL 6 / full model commentary in tier-high (green-bordered) card. Click on March 15 76ers row (pre-pick_history) shows Wager $25 / Win amount $67.50 then falls back to "No model details on file" empty state. Bundesliga 5/3 row renders correctly with BUNDESLIGA tag, "WOB 1, SCF 1" final, $-14 loss. Seeded a placeholder daily summary to verify the "AI" badge + prose block layout under the day header (gold-tinted band, "AI" pill); cleared before commit so no fake content shipped.
- Bundle: `index-D059TUWq.js` 251.48 kB raw / 76.50 kB gzip; `index-2opY7Xk6.css` 18.76 kB raw / 3.73 kB gzip. Bundle parity: `diff -r frontend/dist/assets public/assets` clean. CI bundle guard will pass.
- Em-dash audit on the diffs: clean. Two voice-bearing diffs (commit messages, `.assetsignore` comments) checked.

### What's next

1. **Max action: add three GH Actions repo secrets.** Settings -> Secrets and variables -> Actions:
   - `ANTHROPIC_API_KEY` (from console.anthropic.com): activates AI daily summaries. Cost ~$0.001/day.
   - `CLOUDFLARE_API_TOKEN` (from dash.cloudflare.com/profile/api-tokens, permission Account -> Workers KV Storage -> Read on EDGE_STATE namespace): activates KV bridge.
   - `CLOUDFLARE_ACCOUNT_ID` (visible in the Cloudflare dashboard sidebar): goes with the token.
   Once all three are set, the next 5 AM UTC cron tick (or a workflow_dispatch) will populate everything.
2. **Verify cron end-to-end after secrets are added.** workflow_dispatch the resolve-bets workflow once and confirm the run logs show: "Found N KV state record(s)", "+ KV-synced as pending: ...", resolve_bets grades the 5/4 bets when their games go final, "Generating summary for 2026-05-04 (...)". Commit + push from the bot, Cloudflare deploys, BalanceChart picks up new dates, AI summaries appear under day headers.
3. **Audit the override drift.** $647.11 file vs $588.68 override = ~$58 unexplained. Possible causes: 4/10 manual lifetime_profit overstated; non-model bets on DK; deposits/withdrawals. If the gap closes after the next few days of fresh cron resolves, the file was lagging; if it stays, there's untracked activity.
4. **Future-game scanning** still on backlog from session 9. Real model-side scope.
5. **Polish**: `docs/cloudflare-access-setup.md` rewrite for Zero Trust UI; delete `rebuild/v2-frontend` branch (local + origin); defer `worker/lib/normalize.ts` extract until 3rd consumer.

### Known gotchas surfaced this session

- **Vite mangles export names during minification.** Session 13's `grep -c "americanWinAmount"` sentinel was a bad bundle-staleness check because the function is exported but inlined under a mangled identifier. Real bundle integrity check is `cd frontend && npm run build` then `diff -r frontend/dist/assets public/assets` (which is what the CI guard does).
- **ESPN soccer uses `STATUS_FULL_TIME`, not `STATUS_FINAL`.** Fixed at `resolve_bets.py:53` to accept both. Without this, no soccer game resolves through the cron - 97 stalled entries discovered.
- **scan_edges.py preserves data.json.bets[] across rewrites.** Comment at line 2087: "The scan only adds new picks to the picks[] array, it does NOT touch bets[]." So game-scan and morning-scan workflows can run in parallel with this session's backfill without wiping it.
- **public/data.json and public/bankroll.json are symlinks to repo root.** `public/pick_history.json` and `public/daily_summaries.json` are too. Don't replace any of these with real files; the cron writes to repo root and the symlinks expose the writes through the assets binding.
- **The KV API endpoint requires URL-encoded path components.** Keys like `state:max.sheahan@gmail.com:2026-05-04` contain `:` and `@` that need encoding. `sync_kv_placements.py` uses `urllib.parse.quote(key, safe="")`.

### Important context for next session

- **Override drift is a real-world question, not a code bug.** The data is internally consistent after backfill. If Max's DK balance and the file diverge, suspect untracked DK activity before code.
- **Cron sequence is now: sync_kv_placements -> resolve_bets -> generate_daily_summary -> commit + push.** All three are defensive on missing env vars. If any fails, the others still run.
- **`scripts/backfill_kv_placements.py` is now historical.** Kept in repo for emergency manual re-runs but the going-forward path is `sync_kv_placements.py` in the cron.
- **The Activity tab's join is `${date}|${pick}|${event}` after em-dash strip on both sides.** Mismatched em-dash handling would silently drop the metadata. Same join shape as bankroll.ts's resolvedKeys join, same as PendingTab's resolvedKeys, same as the activity normalizer. Consistency matters.
- **`ActivityResponse.summaries` defaults to `{}` via `z.record(...).default({})`.** Frontend `?? {}` fallback is technically redundant but safe. If the field ever becomes required, drop the frontend fallback then.
- **97 pending soccer entries in `pick_history.json`** will resolve on the next cron tick because of the STATUS_FULL_TIME fix. Expect a large pick_history.json diff on the next auto-resolve commit.

### If you just have one minute, do this
`cd ~/Betting\ Skill && python3 -c "import json; d=json.load(open('data.json')); print(len([b for b in d['bets'] if b.get('outcome')!='pending']), 'resolved,', len([b for b in d['bets'] if b.get('outcome')=='pending']), 'pending'); print(d['bankroll'])"` should print at least `60+ resolved, 7 pending` and a bankroll dict with `record: {wins: 42, losses: 25, pushes: 1}`. If counts are lower, the backfill was rolled back. To exercise the worker enrichment: `curl -s http://localhost:8787/api/activity` (after wrangler-dev is up; doesn't need Access locally). Each bet should have optional `market`, `model`, `edge`, etc. when a pick_history match exists. If `summaries` is `{}`, the AI key isn't set yet - that's expected until Max wires it.
