...
**Rule:** When a flag's intent depends on which side of a market a pick is on (favorite vs underdog, over vs under, home vs away), key it off the structural field the candidate-builder already tracks (here `cand["is_favorite"]`), NOT off a derived proxy like odds sign. Spread juice is shaped by historical cover rate, not by which side the book thinks is more likely to win the game. Same trap waits in totals (juice shaped by O/U side imbalance) and ML markets (juice shaped by sharp/public split).
**Code fix:** scripts/scan_edges.py — get_effective_min_edge now takes `is_favorite: bool` directly; the single call site in calculate_edge passes `cand["is_favorite"]`. Docstring updated to call out the trap. The MLB favorite branch is now dead code (MLB_FAVORITE_HARD_SKIP=True culls -1.5 candidates before this function is reached) but kept for the day that flag flips.
**Backlog:**
- The "Edge below 3% threshold" reason string in `no_edge_games` is hard-coded but the actual floor is sport-specific (5%/6%/8%). Replace with the computed floor or a generic "below threshold" message so audits aren't misleading.
- Verify next 3 days: MLB game picks/day should trend back toward the historical ~12/day baseline on healthy slates. If they stay near zero, the bug isn't the dominant cause and we go to the next candidate (calibration squashing, situational adjustments, ESPN endpoint coverage).
- Audit-time check: write a one-liner that scans pick_history.json for floor-vs-edge inversions (e.g. picks at the boundary of any floor that look mis-classified) so this class of bug is caught at the next audit instead of in-prod.

## 2026-05-03b: Player prop resolution wired up; first 25 props graded [AUTOMATE - DONE]
**Observation:** scripts/fetch_props.py has been emitting NBA player prop picks since at least 2026-04-04, but scripts/resolve_bets.py never knew how to grade them. The pick-detection cascade in resolve_pick_history fell through to "unknown" for any pick that didn't match the moneyline/total/spread shapes, so 29 prop picks sat permanently `pending` for nearly a month. Picks were also being silently lost to the analyzer (analyze_history.py only counts resolved picks, so prop performance was invisible).
**Impact:** With the resolver wired up, 25 of 29 stuck props graded on first run: 16W-9L (64.0% hit) at +$31.91 paper P/L on $10/bet. Sample EV was high — most picks were 15% claimed edge (capped). Wembanyama OVER 12.5 reb hit (he had 18). Jokic UNDER 27.5 pts lost (he had 40). Maxey UNDER 28.5 pts hit (he had 23). The 4 unresolved are edge cases: 2× Jaylen Brown 27.5 referenced TOR @ BOS on a date the game wasn't scheduled (scanner-side bug, scan_date != game date); 2× Tyler Herro 3PT on a night Herro was DNP (would be voided in real betting).
**Rule:** Any new pick-type emitted by the scanner must have a corresponding resolver branch BEFORE the scan ships. Otherwise picks accumulate unresolved and hide their performance from the calibration analyzer. Gate: every market type added to fetch_props.py or scan_edges.py needs a `resolve_*` function and a dispatch entry in resolve_pick_history. Same for the bets[] resolver in main() if/when prop bets get placed.
**Code fix:** scripts/resolve_bets.py — added NBA_PROP_STAT_TO_ESPN dict mapping our pick-string stat labels to ESPN box-score keys; added fetch_nba_player_box(event_id) returning {player: {stat: value}}; added resolve_prop(pick_str, player_box) returning (outcome, detail_str). Modified resolve_pick_history to detect `type == 'prop' or market == 'Player Prop'`, fetch the box for the matching event_id (cached per script run), and grade. Surfaces the actual stat line in final_score so the activity view shows WHY each prop won. fetch_nba_scores rewritten to delegate to fetch_espn_scores so event_id is included in returned game dicts.
**Backlog:**
- Add NHL/MLB prop fetchers when those markets get scanned. Today's prop scanner is NBA-only.
- Handle DNP players: mark as void/push instead of perpetual-pending. Could check if player appears in box at all; if not AND game is final, mark void.
- Handle scan-date != game-date mismatches at the SCANNER side (don't emit picks for games that aren't on the scan_date), or at the RESOLVER side (try +/- 1 day if not found on scan_date).
- Wire prop grading into the placed-bet path (main() at bets[] level) once props start being placed for real.
- Re-audit with prop data included in analyze_history.py output to compare prop EV against game EV.

## 2026-05-03: Per-game candidate selection must use EV-per-$1, not raw edge% [AUTOMATE - DONE]
**Observation:** Audit of pick_history.json shows a structural favorite bias. 71.7% of all 759 resolved picks are on favorites (odds < -100); they hit 56.5% and lose -$294. The 28.3% of picks on underdogs hit 57.7% and win +$942. NBA picks are 100% favorites; MLB is 68.2%; NHL is 50.0%. The +151 to +250 underdog bucket alone is 66.0% hit / +$803 / 75.8% ROI over 106 picks. Every NBA pick we've ever placed has been on the favorite side. Today's only picks were two MLS UNDER 3.5 at -170, illustrating the same pattern: heavy juice on the chalk side.
**Root cause:** scripts/scan_edges.py calculate_edge() evaluates BOTH home and away spread candidates and selects whichever has the higher raw edge%. Skellam/normal-CDF math systematically gives the model-favored side a larger absolute edge for a given line, because cushion-beyond-the-spread is bigger on the side the model agrees with the market about. Edge% alone is the wrong metric: a 5% edge on a +150 dog is +12.5% EV per $1 risked, but a 5% edge on a -150 fav is only +8.3% EV. The April 2026 commit 6629f41 fixed EV-per-$1 sorting at the FINAL display layer but did not fix the per-game candidate picker; underdogs were still being filtered out before they could reach the sort.
**Rule:** When selecting between two mutually-exclusive sides of the same market, rank by expected value per dollar risked, not by raw edge percentage. EV/$1 = model_prob × (decimal_odds - 1) - (1 - model_prob). A min_edge floor still applies to both candidates so we never bet a 1% edge dog just because it pays well; among candidates that clear the floor, the higher-EV side wins.
**Code fix:** scripts/scan_edges.py calculate_edge() — added best_ev tracking alongside best_edge; the candidate-loop selection condition is now `edge >= min_edge and cand_ev > best_ev`. cand_ev computed via american_to_decimal(dk_odds_val) and the EV/$1 formula. best_edge still tracked for downstream notes/sizing. Comment block at line ~1328 documents the rationale.
**Backlog (from today's audit, deferred per one-change-at-a-time):**
- Wire up player prop resolution. fetch_props.py is fully built but pick_history.json shows 29 prop picks all stuck `pending` since 2026-04-04. resolve_bets.py never grades them. Need ESPN box-score-by-player lookup + over/under grading.
- Build futures scanner. Zero coverage today; DK margin on futures is wider than on game lines.
- Investigate -150 to -199 bleed bucket directly. 30.8% of all bets sit here losing -$272. May survive the EV-selection fix; may not. Re-audit after this patch ships.
- The NBA-only 100%-favorites pattern may need its own underdog-allocation rule once the EV picker generalizes; NBA Spreads are now hard-skipped at 8%+ regular-season per yesterday's patch, but in-playoff and sub-8% NBA picks still default to chalk.
...
## 2026-06-09: Judge model calibration from game_log, not pick_history [MODEL]
- A calibration audit on pick_history showed game-line picks predicted 64% but won only 52% (-11.6pp), which looked like the model running hot. The unbiased game_log (every assessed game, including became_pick=false) showed the raw model is only ~-3pp off and the no-vig market is the best single predictor. The big pick-sample gap is ADVERSE SELECTION: picks are the games where the model most disagrees with a sharp DK line, i.e. exactly where it is most wrong.
- **Rule:** Never judge projection quality from pick_history alone; it is adverse-selected. Use game_log (the full unbiased assessment sample). The model barely beats the market, so anchor toward the no-vig market (see market_blend.py) rather than trusting model-alone edges.

## 2026-06-09: bessel_i_n forward recurrence is unstable for low means; use convolution [MODEL]
- skellam_cdf summed a Bessel-based pmf from the far-left tail and broke on the first term below 1e-15, returning 0 for low or asymmetric means. That silently made skellam_sf return a spurious 1.0 on low-scoring run lines (e.g. 2.7 vs 3.4 runs), an active bug in the workhorse run-line market, not just the soccer regime. bessel_i_n's forward recurrence is numerically unstable when its order exceeds its argument (the small-mean regime).
- **Rule:** Compute Skellam / goal-difference probabilities at low means (soccer xG, low run totals) by direct independent-Poisson convolution, not the Bessel recurrence. Validate any new distribution function against a convolution reference that sums to 1 across its support.

## 2026-07-09: Validate a counterfactual replay by reproducing actuals before trusting it [MODEL]
- The first blend-weight replay of the June MLB totals window used the generic 3% edge floor and showed 20 would-be bets at w=0.6 when reality produced 4 picks, a 5x overstatement. The real MLB totals floor is 6% (MLB_TOTAL_MIN_EDGE, raised 2026-06-09). With the correct floor the replay reproduced the actual picks exactly (4 bets, 2-2, -$0.87), which is what licensed the w=0.85 counterfactual (+$70.63) as trustworthy.
- **Rule:** Before reporting any counterfactual pnl, run the replay at the LIVE policy and confirm it reproduces the actual pick set and pnl. If it does not match, the replay is missing a gate (per-sport floors, juice filters, playoff discounts); find it before changing any knob. A counterfactual is only as good as its reproduction of reality.

## 2026-07-09: Local scans need SSL_CERT_FILE; a cert-less run fails soft as "0 games" [MANUAL]
- Running scan_edges.py locally on the Mac failed every ESPN fetch with SSL CERTIFICATE_VERIFY_FAILED (python.org macOS Python ships without linked system certs), and the scan exited cleanly with "0 games, preserving existing picks" — a silent no-op that looks like an empty slate, not an error.
- **Rule:** Invoke local scans as `SSL_CERT_FILE=$(python3 -m certifi) python3 scan_edges.py`. Treat a local scan reporting 0 games across ALL sports as a fetch failure, not a quiet day; check the top of the output for SSL errors before believing it. GitHub Actions is unaffected.
