...
// Per-date AI summary entry. Generated by scripts/generate_daily_summary.py
// inside the resolve-bets cron via the Anthropic API. Empty object when
// generation is disabled (no ANTHROPIC_API_KEY) or when no dates have a
// summary yet. Frontend renders the text under each day-group header.
...
export const ActivityResponseSchema = z.object({
  ...
  // Keyed on YYYY-MM-DD. Worker loads daily_summaries.json and forwards
  // verbatim. Frontend looks up summaries[bet.date] in the day header.
  summaries: z.record(z.string(), DailySummarySchema).default({}),
})
